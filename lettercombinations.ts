const letterCombination = (numberInput : string) : string[] => {
    let result : string[] = []
    let numberMapping : string[] = [" "," ","abc","def","ghi","jkl","mno","pqrs","tuv","wyxz"]
    ...
    if (numberInput.length>1) {
        let numberInputArray: string[] = numberInput.split("");
        for (let letter of numberMapping[Number(numberInputArray[0])]) {
            letter = letter.trim()
            let letterToPush : string = letter
            for (let ctr:number=1;ctr<numberInputArray.length;ctr++) {
