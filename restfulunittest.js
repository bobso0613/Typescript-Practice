const api_call = async (mode, key, params) => {
    let method = "POST";
    let extra_params = "";
    let api_response;
    if (mode == "read") {
...
const test_log = async (mode, key, params) => {
    // Arrange
    const data_type_check = "string";
    const result_check = '{"result":0,"error_message":"error","data":[]}';
    // Act
    const result = JSON.stringify(await api_call(mode, key));
